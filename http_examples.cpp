...
int main() {
   ...
   * Post rtree
   */
  server.resource["^/rtree$"]["POST"] = [&vp](shared_ptr<HttpServer::Response> response, shared_ptr<HttpServer::Request> request) {
	...
	for(auto iter=pt.get_child("polygon").begin();iter!=pt.get_child("polygon").end();iter++)
	{
		ptree::value_type &cell = iter->second;
		int x = cell.second.get_value<int>();
		iter++;
		int y = cell.second.second.get_value<int>();
		point p = {x,y};
		vp.push_back(p);
	}
	/*
	for (ptree::value_type &row : pt.get_child("polygon"))
	{
		for (ptree::value_type &cell : row.second)
		{
			int x = cell.second.get_value<int>();
			int y = cell.second.get_value<int>();
			point p = {x,y};
			vp.push_back(p);
		}
	}*/
